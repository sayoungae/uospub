...
function fnPaging (pageIndex) {
	...
	jQuery("#listForm").attr("action", "/mbaNew/korFree/list.do").submit();
...
function fnView (sort, seq) {
	...
	jQuery("#listForm").attr("action", "/mbaNew/korFree/view.do").submit();
...
function fnWrite () {
	...
	jQuery("#listForm").attr("action", "/mbaNew/korFree/write.do").submit();
...
function fnInsert () {
	...
	if ( confirm("글을 등록 하시겠습니까?") ) {
		...
		jQuery("#frm").attr("action", "/mbaNew/korFree/insert.do").submit();
...
function fnModify () {
	...
	jQuery("#listForm").attr("action", "/mbaNew/korFree/modify.do").submit();
...
function fnUpdate () {
	...
	if ( confirm("글을 수정 하시겠습니까?") ) {
		...
		jQuery("#frm").attr("action", "/mbaNew/korFree/update.do").submit();
...
function fnDelete () {
	...
	if ( confirm("글을 삭제 하시겠습니까?") ) {
		...
		jQuery("#listForm").attr("action", "/mbaNew/korFree/delete.do").submit();
...
function lawFreeComIn () {
	...
	if ( confirm("글을 등록 하시겠습니까?") ) {
		...
		jQuery("#commForm").attr("action", "/mbaNew/Free/commInsert.do").submit();
...
/*댓글 delete*/
function lawFreeComDel (list_id,seq,list_seq) {
	if (jQuery("#lwriteAuth").length > 0) {
		if (jQuery("#lwriteAuth").val() != "Y") {
			alert("권한이 없습니다.");
			return;
	...
	$("#list_seq").val(seq);
	if ( confirm("글을 등록 하시겠습니까?") ) {
		jQuery("#commForm").attr("action", "/Free/commInsert.do").submit();
	} else {
